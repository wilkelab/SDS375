...
We will be using two R packages, **tidyverse** for the data manipulation functions `%>%`, `group_by()`, `summarize()`, `pivot_wider()`, `pivot_longer()`, and `left_join()`, and **palmerpenguins** for the `penguins` dataset.
...
The function `count()` here does all the work for us, but what if instead of counting we wanted to calculate the mean flipper length of the penguins for each species, or calculate the mean flipper length and count at the same time? We need a general framework that allows us to do these kinds of calculations with maximum flexibility.
...
We can perform multiple summaries at once by adding more statements inside the `summarize()` function. To try this out, calculate the mean bill length in addition to the mean body mass.
...
penguins %>%
  group_by(species) %>%
  summarize(
    mean_mass = mean(body_mass_g, ___),
...
Consider the following two versions of a summary table. The first is in long format, where sex is one column and the mean body mass is another. 

```{r echo = TRUE}
penguins_long
```

The second is in wide format, where the values of the sex variable (female or male) are used as column headings, and the mean body mass values are distributed over these two columns.

```{r echo = TRUE}
penguins_wide
```

You can turn a long table into a wide table using the function `pivot_wider()`, which takes two arguments: `names_from` specifies the long column from which the new wide column names should be taken (here e.g., `"sex"`), and `values_from` specifies the long column from which the new wide values should be taken (here e.g., `"mean_mass"`). Try this out.
...
penguins_wide %>%
  pivot_longer(cols = c(female, male), names_to = "sex", values_to = "mean_mass")
```
...
**A few introductory sentences are needed here.**


Consider these two datasets, which contain the population number of three Texas cities and the city area, respectively. (The cities in the two tables are not the same, on purpose.)
