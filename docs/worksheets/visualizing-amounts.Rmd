...
By default, the bars for different `fill` values but identical `x` values will be drawn on top of one-another. But there are other possibilities, which are controlled by the `position` argument to `geom_bar()`. For example, try to set the position to `"dodge"`.
